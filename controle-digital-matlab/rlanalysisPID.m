function varargout = rlanalysisPID(varargin)
% RLANALYSISPID M-file for rlanalysisPID.fig
%      RLANALYSISPID, by itself, creates a new RLANALYSISPID or raises the existing
%      singleton*.
%
%      H = RLANALYSISPID returns the handle to a new RLANALYSISPID or the handle to
%      the existing singleton*.
%
%      RLANALYSISPID('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in RLANALYSISPID.M with the given input arguments.
%
%      RLANALYSISPID('Property','Value',...) creates a new RLANALYSISPID or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before rlanalysisPID_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to rlanalysisPID_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help rlanalysisPID

% Last Modified by GUIDE v2.5 16-Oct-2008 15:34:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @rlanalysisPID_OpeningFcn, ...
                   'gui_OutputFcn',  @rlanalysisPID_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before rlanalysisPID is made visible.
function rlanalysisPID_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to rlanalysisPID (see VARARGIN)

% Planta
Gpc = tf(0.7,[1 1],'iodelay',1); handles.Gpc = Gpc;
...
% Choose default command line output for rlanalysisPID
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes rlanalysisPID wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = rlanalysisPID_OutputFcn(hObject, eventdata, handles) 
...
Gpc = handles.Gpc;
Kp = handles.Kp; Ti = handles.Ti; Td = handles.Td;
f = handles.f; T = 1/f;
...
% Planta
Gpd = c2d(Gpc,T,'zoh');
pp = [roots(cell2mat(Gpd.den)) zeros(1,Gpd.iodelay)]
zp = [roots(cell2mat(Gpd.num))]
% PID
Gcd = tf([(2*T*Ti+T^2+2*Ti*Td) (T^2-2*T*Ti-4*Ti*Td) 2*Ti*Td],2*T*Ti*[1 -1 0],T);
pc = [0; 1]; %já sei quais são...
zc = roots(cell2mat(Gcd.num))
% FTMA
Gma = series(Gcd,Gpd);
% FTMF
Gmf = feedback(series(tf(Kp,1,T),Gma),1);
...
handles.Td = str2double(get(hObject,'String'));
handles.Tdh = hObject;
