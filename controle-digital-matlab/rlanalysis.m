function varargout = rlanalysis(varargin)
% RLANALYSIS M-file for rlanalysis.fig
%      RLANALYSIS, by itself, creates a new RLANALYSIS or raises the existing
%      singleton*.
%
%      H = RLANALYSIS returns the handle to a new RLANALYSIS or the handle to
%      the existing singleton*.
%
%      RLANALYSIS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in RLANALYSIS.M with the given input arguments.
%
%      RLANALYSIS('Property','Value',...) creates a new RLANALYSIS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before rlanalysis_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to rlanalysis_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help rlanalysis

% Last Modified by GUIDE v2.5 07-Oct-2008 16:15:53

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @rlanalysis_OpeningFcn, ...
                   'gui_OutputFcn',  @rlanalysis_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before rlanalysis is made visible.
function rlanalysis_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to rlanalysis (see VARARGIN)

wn = handles.wn; xsi = handles.xsi; f = handles.f;
axes(handles.axes1)
set(gca,'fontsize',8)
zgrid;
hold on
...
% Choose default command line output for rlanalysis
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes rlanalysis wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = rlanalysis_OutputFcn(hObject, eventdata, handles) 
...
[ydeg,tdeg]=step( c2d(tf([wn^2],[1 2*xsi*wn wn^2]),1/f,'match') );
[tt,yy]=stairs(tdeg,ydeg);
plot(tt,yy,'g',[0 10],[1 1],'k');
axis([0 10 0 1.5])
